   ...
   */
    ...
    */
    ...
    _toggleCommentControls(bShow) {
      this._oView.getModel("uiModel").setProperty("/toggleFooterVisibility", bShow);
    ...
    onSaveButtonPress() {
      ...
      aItems.forEach((oItem, iIndex) => {
        ...
        if (!oMessageInput.getValue()) {
          ...
          bValidationFailed = true;
          Messaging.addMessages(new Message({
            message: this._oResourceBundle.getText("mandatoryFieldsMessage"),
            type: MessageType.Error,
      ...
      if (bValidationFailed) {
        MessageToast.show(this._oResourceBundle.getText("mandatoryFieldsMessage"),);
        return;
      }

      if (oModel.hasPendingChanges() && (!mData.Name || !mData.Specs || !mData.Price_amount)) {
        oModel.setRefreshAfterChange(false);
        oModel.submitChanges({});
      } else if (oModel.hasPendingChanges() && (mData.Name || mData.Specs || mData.Price_amount)) {
        oModel.setRefreshAfterChange(true);
        oModel.submitChanges({
          success: () => {
            BusyIndicator.hide();
